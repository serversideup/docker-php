...
- Publishing is three phases, each a reusable workflow the callers (`action_publish-images-*.yml`) chain: `service_setup-matrix.yml` generates the matrix once; `service_build-images.yml` builds one variation and saves every image to the Depot Registry (nothing public yet); `service_smoke-test.yml` pulls a slice of those saved images and runs `scripts/smoke-test.sh` on native amd64 and arm64 runners; `service_publish-images.yml` promotes the exact tested images by digest to Docker Hub and GHCR with `docker buildx imagetools create`, and only runs if smoke passed. `service_summary.yml` renders the run table last. Jobs render as `cli / 8.4.25-bookworm`, `smoke / cli 8.4.25-bookworm arm64`, `publish / cli 8.4.25-bookworm`.
- A broken image can never reach a public tag: what users pull is byte-for-byte what the smoke tests ran. Saved images live in the Depot Registry until the project's retention setting removes them.
...
- Depot authentication: the project ID lives in `depot.json` (not a secret). Same-repo runs authenticate through a Depot OIDC trust relationship (`id-token: write`). Pull requests from forks have no OIDC token, so Depot falls back to its open-source pull request flow: the full matrix builds on isolated builders and is neither saved nor published. A maintainer publishes a fork's images to `serversideup/php-dev` by running the "Docker Publish (PR Images)" workflow manually with the PR number.
- CI helper scripts (`scripts/build-summary.sh`, `scripts/select-smoke-images.sh`) are checked by `scripts/tests/run.sh`, a single file of plain assertions over sample images. Run it after changing them and add an assertion when you add behavior. Keep jq programs explicit with parentheses: `a + b as $x | ...` parses differently across jq versions, and runners ship an older jq than most laptops.
...
- `scripts/smoke-test.sh` checks that an image starts, runs as an unprivileged user, reports the expected PHP version, and reaches a healthy HEALTHCHECK. Run it locally against any image before opening a PR that touches startup behavior.
- Every build job records its image details (tags, Depot Registry reference) as an artifact; the publish job updates it with the compressed size per architecture. `service_summary.yml` merges them with `scripts/build-summary.sh` into one table on the run summary and in the PR comment.
