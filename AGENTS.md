...
- Publishing is three phases, each a reusable workflow the callers (`action_publish-images-*.yml`) chain: `service_setup-matrix.yml` generates the matrix once; `service_build-images.yml` builds one variation and saves every image to the Depot Registry (nothing public yet); `service_smoke-test.yml` pulls every saved image and runs `scripts/smoke-test.sh` against it on native amd64 and arm64 runners; `service_publish-images.yml` promotes the exact tested images by digest to Docker Hub and GHCR with `docker buildx imagetools create`, and only runs if smoke passed. `service_summary.yml` renders the run table last. Jobs render as `cli / 8.4.25-bookworm`, `smoke / cli 8.4.25-bookworm arm64`, `publish / cli 8.4.25-bookworm`.
- A broken image can never reach a public tag: what users pull is byte-for-byte what the smoke tests ran. Publishing is all or nothing: if any image fails to build or fails its smoke test, no image from that run is published, including on the weekly rebuilds. A red run therefore always needs a human look. Saved images live in the Depot Registry until the project's retention setting removes them.
