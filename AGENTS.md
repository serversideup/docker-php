...
- Publishing is three phases, each a reusable workflow the callers (`action_publish-images-*.yml`) chain: `service_setup-matrix.yml` generates the matrix once; `service_build-images.yml` builds one variation and saves every image to the Depot Registry (nothing public yet); `service_test-images.yml` pulls every saved image and runs `scripts/test-image.sh` against it on native amd64 and arm64 runners; `service_publish-images.yml` promotes the exact tested images by digest to Docker Hub and GHCR with `docker buildx imagetools create`, and only runs if the tests passed. `service_report.yml` renders the run table last. Jobs render as `cli / 8.4.25-bookworm`, `test / cli 8.4.25-bookworm arm64`, `publish / cli 8.4.25-bookworm`.
...
- `scripts/test-image.sh` checks that an image starts, runs as an unprivileged user, reports the expected PHP version, and reaches a healthy HEALTHCHECK. Run it locally against any image before opening a PR that touches startup behavior.
- Every build job records its image details (tags, Depot Registry reference) as an artifact; the publish job updates it with the compressed size per architecture. `service_report.yml` merges them with `scripts/build-summary.sh` into one table on the run summary and in the PR comment.
