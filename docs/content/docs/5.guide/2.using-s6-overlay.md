...
- ❌ S6 Overlay may not be compatible with all PaaS providers, depending on how they run their containers ([see this comment from the S6 Overlay creator](https://github.com/just-containers/s6-overlay/issues/535#issuecomment-1597680218){target="_blank"})
...
:u-button{to="/docs/customizing-the-image/changing-common-php-settings" label="Changing Common PHP Settings" aria-label="Changing Common PHP Settings" size="md" color="primary" variant="outline" trailing-icon="i-lucide-arrow-right" class="font-bold"}