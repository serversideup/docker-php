...
|-------------|-----------------|----------------------|------------------------|
| `libfcgi-bin`<br/>(Debian)<br/>`fcgi`<br/>(Alpine) | FastCGI is a protocol for interfacing interactive programs with a web server. | *-fpm-nginx <br/> *-fpm-apache | This is required for the webserver to interface with PHP-FPM |
