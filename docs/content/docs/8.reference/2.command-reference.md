...
Packages can be passed as separate arguments, as a comma-separated list, or split across multiple lines with backslash continuations:

```bash [Example - Installing multiple packages on Alpine]
# Separate arguments
docker-php-serversideup-dep-install-alpine git zip unzip

# Comma-separated
docker-php-serversideup-dep-install-alpine "git, zip, unzip"

# Backslash continuations (useful in a Dockerfile)
docker-php-serversideup-dep-install-alpine \
    git \
    zip \
    unzip
...
```bash [Example - Installing multiple packages on Debian]
# Separate arguments
docker-php-serversideup-dep-install-debian git zip unzip
