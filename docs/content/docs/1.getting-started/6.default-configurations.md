...
| [opentelemetry](https://github.com/open-telemetry/opentelemetry-php-instrumentation) | Provides the hooks that the OpenTelemetry SDK uses to automatically instrument your application for distributed tracing. | Vendor-neutral observability without running a separate APM agent process, which avoids conflicting with S6 Overlay. The extension is inert until you install the OpenTelemetry SDK via Composer, and our tests concluded it only added about 0.2MB to the image size.<br /><br />ℹ️ Requires PHP 8.0+, so it is **not installed on PHP 7.4 images**. See our [OpenTelemetry guide](/docs/customizing-the-image/opentelemetry).|
