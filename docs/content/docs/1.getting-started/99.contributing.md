...
- **Nothing is published until it passes a smoke test.** Every build is saved to a private registry first. The newest PHP version of every variation is pulled from there on native amd64 and arm64 runners and checked: it starts, runs unprivileged, reports the right PHP version, and becomes healthy. Only then is the exact same image promoted to Docker Hub, so what you pull is what was tested. You can run the same check locally with `bash scripts/smoke-test.sh serversideup/php-dev:<tag>`.
