...
- **Pull requests from forks** are built on isolated builders and are never pushed to a registry. A green check means your change builds everywhere.
- **Pull requests from this repository** are also published to `serversideup/php-dev` with your PR number as the tag prefix (for example `serversideup/php-dev:699-8.4-fpm-nginx-alpine`). A comment on the PR lists the pull commands.
- **Every run ends with a summary table** listing each image with its compressed size per architecture, how much it grew compared to the current production image, and a pull command. Growth over 10% is flagged.
- **Published images are smoke tested.** The newest PHP version of every variation is pulled on native amd64 and arm64 runners and checked: it starts, runs unprivileged, reports the right PHP version, and becomes healthy. You can run the same check locally with `bash scripts/smoke-test.sh serversideup/php-dev:<tag>`.
- **Transient failures heal themselves.** If a few jobs fail because a registry or mirror hiccupped, CI re-runs just those jobs once. If the run is still red after that, something in the change needs a look.
