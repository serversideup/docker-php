...
- **Pull requests from forks** are built on isolated builders and never leave them. A green check means your change builds everywhere.
- **Pull requests from this repository** go through the same three phases as a release: build, smoke test, publish. Images are published to `serversideup/php-dev` with your PR number as the tag prefix (for example `serversideup/php-dev:699-8.4-fpm-nginx-alpine`) only after the smoke tests pass. A comment on the PR shows the status and the pull commands.
...
- **Nothing is published until it passes a smoke test.** Every build is saved to a private registry first. Every image is pulled from there on native amd64 and arm64 runners and checked: it starts, runs unprivileged, reports the right PHP version, and becomes healthy. Only then is the exact same image promoted to Docker Hub, so what you pull is what was tested. If any image fails to build or fails its smoke test, nothing from that run is published. You can run the same check locally with `bash scripts/smoke-test.sh serversideup/php-dev:<tag>`.
