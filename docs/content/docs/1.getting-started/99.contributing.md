...
Every workflow change is checked with [actionlint](https://github.com/rhysd/actionlint) in CI. Run the same check locally before you push:

```bash [Terminal]
docker run --rm -v "$PWD:/repo" -w /repo rhysd/actionlint:latest -color
```

The Depot runner labels we use are listed in `.github/actionlint.yaml` so actionlint recognizes them.
